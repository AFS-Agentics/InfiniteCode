...
`infinitecode onboard` 是推薦的設定路徑。如需手動配置，InfiniteCode 會按以下順序合併設定：

1. 內建預設值
2. `INFINITECODE_HOME/config.toml` - 使用者級配置，預設在 macOS/Linux 上為
   `~/.infinitecode/config.toml`，在 Windows 上為 `C:\Users\yourname\.infinitecode\config.toml`
3. `<workspace>/.infinitecode/config.toml` - 專案級配置
4. CLI flags

憑據單獨保存在 `INFINITECODE_HOME/auth.json`；`config.toml` 應引用 credential id，
而不是直接儲存 API key。
...
- `model_slug` 從 `models.json` 中選擇 InfiniteCode 的本地模型中繼資料。
- `provider` 選擇已配置的連線記錄。
- `request_model` 是傳送到 provider 的特定模型字串。
...
專案級覆蓋也可以放在 `<workspace>/.infinitecode/models.json`。
在 `models.json` 中，`provider` 是該模型的預設 wire API 中繼資料；實際端點仍由
`config.toml` 中的 `provider` 欄位選擇。
若省略 `base_instructions`，InfiniteCode 會回退到內建預設 base instructions；明確寫空字串
