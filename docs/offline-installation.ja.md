...
InfiniteCode のインストーラーはオフラインモードをサポートしており、必要なすべてのアセットをインストーラースクリプトと同じディレクトリから読み込み、ネットワークには接続しません。

インターネットにアクセスできるマシンで:

1. インストーラースクリプトをダウンロードします。Linux/macOS は `install.sh`、Windows は `install.ps1` です。
2. 対象 CPU と OS 向けの最新 InfiniteCode release asset をダウンロードします。例: `x86_64`
   と `aarch64`/`arm64`。
...
オフラインモードでは、モデルは
`<INFINITECODE_HOME>/local-models/minishlab--potion-code-16M` にインストールされます。
これはランタイムの code-search provider が使用するディレクトリです。
`INFINITECODE_HOME` が設定されていない場合は
`~/.infinitecode/local-models/minishlab--potion-code-16M` になります。
