...
**InfiniteCode - open-source coding agent с Desktop app, terminal TUI/CLI и модельно-нейтральным Rust runtime для приватных, корпоративных и OpenAI-совместимых модельных сред. Подключайте DeepSeek, Qwen, Kimi, Anthropic-совместимые API, локальные шлюзы или собственные endpoint моделей.**
...
[Почему InfiniteCode](#почему-infinitecode) · [Скриншоты](#скриншоты) · [Возможности](#возможности) · [Проверенные модели](#проверенные-модели) · [Проверенные платформы](#проверенные-платформы) · [Установка](#установка) · [Быстрый старт](#быстрый-старт) · [Документация](#docs)
...
<p align="center">
  <img width="100%" alt="InfiniteCode desktop coding agent app показывает беседу по репозиторию, боковую панель проекта и управление моделью" src="./.github/assets/infinitecode-desktop-coding-agent-screenshot.png" />
</p>

<p align="center">
  <img width="100%" alt="InfiniteCode terminal TUI coding agent работает в локальном репозитории и показывает модель, контекст и token status" src="./.github/assets/infinitecode-terminal-tui-coding-agent-screenshot.png" />
</p>

## Почему InfiniteCode

InfiniteCode предназначен для команд, которым нужен coding agent вне единой hosted
...
- **Подходит для приватных и intranet-сред** - InfiniteCode запускается как единый
...
- **Поддержка долгих задач** - Позвольте InfiniteCode автоматически управлять контекстом
...
Встроенный каталог моделей InfiniteCode содержит проверенные определения моделей для
...
InfiniteCode протестирован на macOS, Linux, Windows и Kylin OS.
...
InfiniteCode можно установить в двух формах. Выберите Desktop app для графического
...
Начните здесь, если хотите использовать графический интерфейс InfiniteCode. Скачайте
последний InfiniteCode Desktop package со страницы
[GitHub Releases](https://github.com/AFS-Agentics/InfiniteCode/releases/latest), затем
выберите asset для вашей операционной системы и архитектуры:

- **macOS** - скачайте `.dmg` или `.zip` asset вида
  `infinitecode-desktop-...-mac-...`.
- **Windows** - скачайте `.exe` asset вида `infinitecode-desktop-...-windows-...`.
- **Linux** - скачайте `.AppImage`, `.deb` или `.rpm` asset вида
  `infinitecode-desktop-...-linux-...`.

**Если macOS сообщает, что `InfiniteCode.app` повреждено и не может быть открыто, это
...
Установите terminal-native команду `infinitecode`, если предпочитаете TUI, хотите
shell automation или хотите использовать InfiniteCode вместе с Desktop app.
...
Онлайн-установщик размещает `infinitecode` в InfiniteCode home directory, устанавливает
...
```bash
infinitecode                         # запустить интерактивный TUI в текущем репозитории
infinitecode resume <session-id>
```

## Конфигурация

`infinitecode onboard` - рекомендуемый путь настройки. Пути ручного `config.toml`,
...
InfiniteCode находится на стадии pre-1.0 и активно развивается. Он готов для локальной
...
**Если InfiniteCode оказался полезен, пожалуйста, поставьте ему star.**
