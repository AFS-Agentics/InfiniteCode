...
//! | `INFINITECODE_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
/// Set the user-configured syntax theme override and infinitecode home path.
///
/// Call this with the **final resolved config** (after onboarding, resume, and
/// fork reloads complete). The first call persists `name` and `infinitecode_home` in
...
pub(crate) fn set_theme_override(
    name: Option<String>,
    infinitecode_home: Option<PathBuf>,
) -> Option<String> {
    let warning = validate_theme_name(name.as_deref(), infinitecode_home.as_deref());
    let override_set_ok = THEME_OVERRIDE.set(name.clone()).is_ok();
    let infinitecode_home_set_ok = CLAWR_HOME.set(infinitecode_home.clone()).is_ok();
    if THEME.get().is_some() {
        set_syntax_theme(resolve_theme_with_override(
            name.as_deref(),
            infinitecode_home.as_deref(),
        ));
    }
    if !override_set_ok || !infinitecode_home_set_ok {
...
pub(crate) fn validate_theme_name(
    ...
    let custom_theme_path_display = infinitecode_home
        ...
        .unwrap_or_else(|| format!("$INFINITECODE_HOME/themes/{name}.tmTheme"));
    ...
    if let Some(home) = infinitecode_home {
...
fn load_custom_theme(name: &str, infinitecode_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, infinitecode_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, infinitecode_home: Option<&Path>) -> Theme {
...
pub(crate) fn resolve_theme_by_name(name: &str, infinitecode_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
    // Bundled theme?
...
/// found in `{infinitecode_home}/themes/`.
pub(crate) fn list_available_themes(infinitecode_home: Option<&Path>) -> Vec<ThemeEntry> {
...
<key>background</key><string>{deleted_background}</string>
    ...
    fn unique_foreground_colors_for_theme(theme_name: &str) -> Vec<String> {
        let theme = resolve_theme_by_name(theme_name, /*infinitecode_home*/ None)
    ...
    fn bundled_theme_can_provide_diff_scope_backgrounds() {
        let theme = resolve_theme_by_name("github", /*infinitecode_home*/ None)
    ...
    fn validate_theme_name_none_for_bundled() {
        ...
        assert!(validate_theme_name(Some("dracula"), /*infinitecode_home*/ None).is_none());
        assert!(validate_theme_name(Some("nord"), Some(Path::new("/nonexistent"))).is_none());
    }

    #[test]
    fn validate_theme_name_none_when_no_override() {
        assert!(validate_theme_name(/*name*/ None, /*infinitecode_home*/ None).is_none());
    }
