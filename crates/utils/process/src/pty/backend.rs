...
use super::process::PtyMasterHandle;
    ...
    fn kill(&mut self) -> std::io::Result<()> {
        ...
        if let Some(process_group_id) = self.process_group_id {
            ...
            let process_group_kill_result =
                super::process_group::kill_process_group(process_group_id);
...
impl ChildTerminator for RawPidTerminator {
    fn kill(&mut self) -> std::io::Result<()> {
        super::process_group::kill_process_group(self.process_group_id)
    }
}
...
fn platform_native_pty_system() -> Box<dyn portable_pty::PtySystem + Send> {
    #[cfg(windows)]
    {
        Box::new(super::win::ConPtySystem::default())
    }
