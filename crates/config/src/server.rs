...
pub struct ServerConfig {
    ...
    /// HTTP coordination layer entirely (Freebuff-style endpoints will
    /// not be served).
    ...
    /// Freebuff-compatible HTTP bridge configuration.
    ///
    /// Mirrors the placement of the Freebuff coordination API on top of the
    ...
    /// `[freebuff_bridge]` `enabled = true` in `config.toml` *and* add at
    /// least one entry to `server.http_listen`.
    #[serde(default)]
    pub freebuff_bridge: FreebuffBridgeConfig,
}
...
/// Coordinates the Freebuff-shaped HTTP bridge that sits on top of the
...
/// the Freebuff / Codebuff backend exposes, so a Freebuff-compatible client
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct FreebuffBridgeConfig {
    ...
    /// `401 AUTH_REQUIRED`. Setting this via the `INFINITECODE_FREEBUFF_BRIDGE_PASSWORD`
    ...
    /// Default 1h (matches `FREEBUFF_GLM_V52_SESSION_LENGTH_MS` semantics).
...
impl Default for FreebuffBridgeConfig {
    fn default() -> Self {
        Self {
            enabled: false,
            password: None,
