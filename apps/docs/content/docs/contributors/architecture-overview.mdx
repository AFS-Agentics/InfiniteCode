...
description: High-level map of InfiniteCode crates and runtime boundaries.
---

InfiniteCode is a Rust workspace built around one interactive path: the `infinitecode` CLI
...
```mermaid
flowchart TD
  CLI["crates/cli<br/>infinitecode command"]
  ...
  Store["infinitecode.db + rollout files<br/>sessions and history"]
...
| --- | --- |
| `infinitecode-cli` | Parses `infinitecode`, `infinitecode onboard`, `infinitecode resume`, `infinitecode prompt`, `infinitecode doctor`, `infinitecode upgrade`, and `infinitecode server`. |
| `infinitecode-arg0` | Lets one binary dispatch by executable name, so `infinitecode` and server-style entrypoints can share one executable. |
| `infinitecode-tui` | Interactive terminal UI, composer, onboarding, model picker, slash commands, transcript rendering, and worker event handling. |
| `infinitecode-server` | Transport-neutral runtime, session lifecycle, event protocol, persistence, compaction, goals, subagents, reference search, shell execution, and provider/vendor APIs. |
| `infinitecode-core` | Agent loop, message/session model, context pipeline, instruction discovery, permissions, hooks, tool registry, query execution, and compaction helpers. |
| `infinitecode-provider` | Provider router and OpenAI/Anthropic-compatible streaming adapters. |
| `infinitecode-protocol` | Shared request, response, event, provider, permission, reference-search, command-exec, goal, and session types. |
| `infinitecode-config` | Serializable config contract for app, provider, server, tool, hook, MCP, skill, and logging settings. |
| `infinitecode-tools` | Tool contracts shared by core, server, and clients. |
| `infinitecode-mcp` | RMCP-backed MCP runtime adapter. |
| `infinitecode-code-search` | Local semantic code retrieval implementation. |
| `infinitecode-file-search` | Fuzzy file search used by composer reference search. |
| `infinitecode-safety` | Approval and sandbox policy contracts. |

## Startup Flow

When a user runs `infinitecode`:

1. `crates/cli/src/main.rs` parses the command.
2. The default command calls `run_agent` in `crates/cli/src/agent_command.rs`.
3. `run_agent` resolves the current working directory, `INFINITECODE_HOME`, effective
   config, model catalog, permission preset, provider settings, and saved model
   entries.
4. `run_agent` calls `infinitecode_tui::run_interactive_tui`.
...
`infinitecode onboard` follows the same setup path, forces provider onboarding mode,
and exits after the provider/model binding is saved. The CLI then prompts the
user to run `infinitecode` so the next process starts from the freshly written config.

`infinitecode resume <session_id>` follows the same path with an initial session id.
...
the core query runtime. The shared tool contract lives in `infinitecode-tools`; concrete
runtime behavior lives under `infinitecode-core/src/tools`.
...
1. `infinitecode-config` defines the serialized provider and model binding fields.
2. `infinitecode-core` resolves effective provider settings for the current workspace.
3. `infinitecode-server` builds the provider router during bootstrap.
4. `infinitecode-provider` sends requests through the selected wire API.
...
Keep UI state in `infinitecode-tui`, runtime/session state in `infinitecode-server`, agent and
tool semantics in `infinitecode-core`, provider-specific request/stream handling in
`infinitecode-provider`, and serialized config shapes in `infinitecode-config`.

When a feature crosses layers, change the shared DTO in `infinitecode-protocol` first,
