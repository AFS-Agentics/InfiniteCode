---
title: 安装
description: 在线安装、源码构建或离线安装 InfiniteCode。
---

InfiniteCode 以用户本地二进制形式安装。安装器也会安装 `rg` sidecar，用于快速仓库搜索。如果你需要本地语义 `code_search`，可以在安装时下载小型本地模型，或使用下面的离线资源包。

## Linux 和 macOS

运行在线安装器：

```bash
curl -fsSL https://raw.githubusercontent.com/AFS-Agentics/InfiniteCode/main/install.sh | sh
```

安装指定版本：

```bash
curl -fsSL https://raw.githubusercontent.com/AFS-Agentics/InfiniteCode/main/install.sh | sh -s -- --version v0.1.2
```

同时安装本地 `code_search` 模型：

```bash
curl -fsSL https://raw.githubusercontent.com/AFS-Agentics/InfiniteCode/main/install.sh | sh -s -- --install-code-search-model
```

Unix 安装器支持：

| 选项 | 用途 |
| --- | --- |
| `--version <version>` | 固定 InfiniteCode 版本，而不是安装最新 release。 |
| `--binary <path>` | 安装本地 `infinitecode` 二进制，而不是下载 release archive。 |
| `--install-dir <dir>` | 安装到自定义目录。 |
| `--install-code-search-model` | 下载 `code_search` 使用的本地 Hugging Face 模型。 |
| `--offline` | 从 `install.sh` 旁边的文件离线安装。 |
| `--no-modify-path` | 不更新 shell profile。 |

常用环境变量：

| 变量 | 效果 |
| --- | --- |
| `VERSION` | 等同于 `--version`。 |
| `INFINITECODE_INSTALL_DIR` | 等同于 `--install-dir`。 |
| `INFINITECODE_SKIP_RG_INSTALL=1` | 跳过内置 `rg` sidecar 安装。 |
| `INFINITECODE_INSTALL_CODE_SEARCH_MODEL=1` | 等同于 `--install-code-search-model`。 |

## Windows

在 PowerShell 中运行在线安装器：

```powershell
irm 'https://raw.githubusercontent.com/AFS-Agentics/InfiniteCode/main/install.ps1' | iex
```

安装指定版本：

```powershell
$env:VERSION = "v0.1.2"; irm 'https://raw.githubusercontent.com/AFS-Agentics/InfiniteCode/main/install.ps1' | iex
```

同时安装本地 `code_search` 模型：

```powershell
$env:INFINITECODE_INSTALL_CODE_SEARCH_MODEL = "1"; irm 'https://raw.githubusercontent.com/AFS-Agentics/InfiniteCode/main/install.ps1' | iex
```

Windows 安装器会安装到当前用户的本地程序目录，并更新用户 `Path`。不需要管理员 shell。

## 从源码构建

贡献 InfiniteCode 或测试未发布修改时使用此路径：

```bash
git clone https://github.com/AFS-Agentics/InfiniteCode.git
cd infinitecode
cargo build --release
./target/release/infinitecode --version
```

在 Linux 和 macOS 上，可以用安装器安装这个本地二进制：

```bash
./install.sh --binary ./target/release/infinitecode
```

## 升级

升级已有在线安装：

```bash
infinitecode upgrade
```

Linux 和 macOS 上，`infinitecode upgrade` 会下载并运行 `install.sh`。Windows 上，它会启动一个独立的 PowerShell 安装器，因为 Windows 不能在当前进程仍持有 `infinitecode.exe` 时替换该文件。

## 无网络安装

企业机器、内网主机或安装时不能访问 GitHub 和 Hugging Face 的机器，使用离线安装。

先在有网络的机器上准备文件，再移动到目标机器。所有文件都放在安装脚本旁边。

下载来源：

| 资源 | 来源 |
| --- | --- |
| Unix 安装器 | `https://raw.githubusercontent.com/AFS-Agentics/InfiniteCode/main/install.sh` |
| Windows 安装器 | `https://raw.githubusercontent.com/AFS-Agentics/InfiniteCode/main/install.ps1` |
| InfiniteCode release archive | `https://github.com/AFS-Agentics/InfiniteCode/releases` |
| ripgrep release archive | `https://github.com/BurntSushi/ripgrep/releases` |
| `code_search` 模型文件 | `https://huggingface.co/minishlab/potion-code-16M/tree/main` |

Linux 和 macOS 所需文件：

| 文件 | 模式 |
| --- | --- |
| 安装器 | `install.sh` |
| InfiniteCode release archive 或二进制 | `infinitecode-*-${target}.tar.gz` 或 `infinitecode` |
| ripgrep archive 或二进制 | `ripgrep-*-${rg_target}.tar.gz` 或 `rg` |
| `code_search` 模型文件 | `config.json`, `model.safetensors`, `tokenizer.json` |

Windows 所需文件：

| 文件 | 模式 |
| --- | --- |
| 安装器 | `install.ps1` |
| InfiniteCode release archive 或二进制 | `infinitecode-*-${target}.zip` 或 `infinitecode.exe` |
| ripgrep archive 或二进制 | `ripgrep-*-${rg_target}.zip` 或 `rg.exe` |
| `code_search` 模型文件 | `config.json`, `model.safetensors`, `tokenizer.json` |

`code_search` 文件来自 `minishlab/potion-code-16M` Hugging Face 模型。它们可以直接放在安装脚本旁边，也可以放在 `minishlab--potion-code-16M/` 子目录下。离线安装器当前会无条件安装这些模型文件；它们是 `code_search` 使用的本地语义搜索模型。

## 离线资源包 target

选择 release archive 时使用下面的 target 字符串。

| 机器 | InfiniteCode `${target}` | ripgrep `${rg_target}` |
| --- | --- | --- |
| Linux x86_64 | `x86_64-unknown-linux-musl` | `x86_64-unknown-linux-musl` |
| Linux arm64 | `aarch64-unknown-linux-musl` | `aarch64-unknown-linux-gnu` |
| macOS Intel | `x86_64-apple-darwin` | `x86_64-apple-darwin` |
| macOS Apple Silicon | `aarch64-apple-darwin` | `aarch64-apple-darwin` |
| Windows 64-bit | `x86_64-pc-windows-msvc` | `x86_64-pc-windows-msvc` |

macOS Apple Silicon 上 InfiniteCode `v0.1.2` 和 ripgrep `14.1.1` 的 Unix 资源包示例：

```text
install.sh
infinitecode-v0.1.2-aarch64-apple-darwin.tar.gz
ripgrep-14.1.1-aarch64-apple-darwin.tar.gz
minishlab--potion-code-16M/config.json
minishlab--potion-code-16M/model.safetensors
minishlab--potion-code-16M/tokenizer.json
```

Windows 资源包示例：

```text
install.ps1
infinitecode-v0.1.2-x86_64-pc-windows-msvc.zip
ripgrep-14.1.1-x86_64-pc-windows-msvc.zip
minishlab--potion-code-16M/config.json
minishlab--potion-code-16M/model.safetensors
minishlab--potion-code-16M/tokenizer.json
```

Archive URL 模式：

```text
https://github.com/AFS-Agentics/InfiniteCode/releases/download/<version>/infinitecode-<version>-<target>.tar.gz
https://github.com/AFS-Agentics/InfiniteCode/releases/download/<version>/infinitecode-<version>-<target>.zip
https://github.com/BurntSushi/ripgrep/releases/download/<rg_version>/ripgrep-<rg_version>-<rg_target>.tar.gz
https://github.com/BurntSushi/ripgrep/releases/download/<rg_version>/ripgrep-<rg_version>-<rg_target>.zip
https://huggingface.co/minishlab/potion-code-16M/resolve/main/config.json
https://huggingface.co/minishlab/potion-code-16M/resolve/main/model.safetensors
https://huggingface.co/minishlab/potion-code-16M/resolve/main/tokenizer.json
```

Linux 或 macOS 离线安装：

```bash
sh ./install.sh --offline
```

Windows 离线安装：

```powershell
.\install.ps1 -Offline
```

离线模式会把模型安装到：

```text
<INFINITECODE_HOME>/local-models/minishlab--potion-code-16M
```

如果未设置 `INFINITECODE_HOME`，InfiniteCode 使用默认用户配置目录：

| 平台 | 默认 `INFINITECODE_HOME` |
| --- | --- |
| macOS/Linux | `~/.infinitecode` |
| Windows | `C:/Users/your-username/.infinitecode` |

完全离线的机器应在 `<INFINITECODE_HOME>/config.toml` 中关闭启动更新检查：

```toml
[updates]
enabled = false
check_on_startup = false
```

见 [配置参考](/zh/docs/reference/config-reference#update-checks)。

## 验证

打开新终端并运行：

```bash
infinitecode --version
infinitecode doctor
```

如果找不到 `infinitecode`，把安装目录加入 `PATH` 后重新打开终端。如果仓库搜索出现 `rg` 错误，重新运行安装器，或把 `rg` 安装到 `infinitecode` 二进制旁边。
