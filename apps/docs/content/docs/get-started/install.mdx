---
title: Install
description: Install InfiniteCode online, from source, or in an offline environment.
---

InfiniteCode installs as a user-local binary. The installer also installs the `rg`
...
| Option | Use when |
| --- | --- |
| `--version <version>` | Pin the InfiniteCode release instead of installing the latest release. |
| `--binary <path>` | Install a local `infinitecode` binary instead of downloading a release archive. |
...
| `INFINITECODE_SKIP_RG_INSTALL=1` | Skip installing the bundled `rg` sidecar. |
| `INFINITECODE_INSTALL_CODE_SEARCH_MODEL=1` | Same as `--install-code-search-model`. |
...
Use this path when you are contributing to InfiniteCode or testing unreleased changes:

```bash
git clone https://github.com/AFS-Agentics/InfiniteCode.git
cd infinitecode
cargo build --release
./target/release/infinitecode --version
```
...
On Linux and macOS, `infinitecode upgrade` downloads `install.sh` and runs it. On
Windows, it starts a detached PowerShell installer because Windows cannot
replace the running `infinitecode.exe` while the current process still owns the file.
...
| Asset | Source |
| --- | --- |
| Unix installer | `https://raw.githubusercontent.com/AFS-Agentics/InfiniteCode/main/install.sh` |
| Windows installer | `https://raw.githubusercontent.com/AFS-Agentics/InfiniteCode/main/install.ps1` |
| InfiniteCode release archive | `https://github.com/AFS-Agentics/InfiniteCode/releases` |
...
| InfiniteCode release archive or binary | `infinitecode-*-${target}.tar.gz` or `infinitecode` |
...
| InfiniteCode release archive or binary | `infinitecode-*-${target}.zip` or `infinitecode.exe` |
...
| Machine | InfiniteCode `${target}` | ripgrep `${rg_target}` |
| --- | --- | --- |
...
Example Unix bundle for InfiniteCode `v0.1.2` and ripgrep `14.1.1` on macOS Apple
Silicon:

```text
install.sh
infinitecode-v0.1.2-aarch64-apple-darwin.tar.gz
...
```text
install.ps1
infinitecode-v0.1.2-x86_64-pc-windows-msvc.zip
...
```text
https://github.com/AFS-Agentics/InfiniteCode/releases/download/<version>/infinitecode-<version>-<target>.tar.gz
https://github.com/AFS-Agentics/InfiniteCode/releases/download/<version>/infinitecode-<version>-<target>.zip
...
```text
<INFINITECODE_HOME>/local-models/minishlab--potion-code-16M
```
...
If `infinitecode` is not found, add the install directory to `PATH` and reopen the
terminal. If repository search fails with an `rg` error, rerun the installer or
install `rg` next to the `infinitecode` binary.
