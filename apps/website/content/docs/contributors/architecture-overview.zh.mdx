---
title: 架构概览
description: InfiniteCode crate 和运行时边界的高层地图。
---

InfiniteCode 是一个 Rust workspace，围绕一条交互路径构建：`infinitecode` CLI 启动 TUI，TUI 将用户动作发送给后台 worker，worker 与 server protocol 通信，server runtime 驱动 sessions、tools、persistence、goals、subagents 和 model providers。
...
| --- | --- |
| `infinitecode-cli` | 解析 `infinitecode`、`infinitecode onboard`、`infinitecode resume`、`infinitecode prompt`、`infinitecode doctor`、`infinitecode upgrade` 和 `infinitecode server`。 |
| `infinitecode-arg0` | 允许一个 binary 按可执行文件名分发，因此 `infinitecode` 和 server-style entrypoint 可以共享同一个 executable。 |
| `infinitecode-tui` | 交互式终端 UI、composer、onboarding、model picker、slash commands、transcript rendering 和 worker event handling。 |
| `infinitecode-server` | Transport-neutral runtime、session lifecycle、event protocol、persistence、compaction、goals、subagents、reference search、shell execution 和 provider/vendor APIs。 |
| `infinitecode-core` | Agent loop、message/session model、context pipeline、instruction discovery、permissions、hooks、tool registry、query execution 和 compaction helpers。 |
| `infinitecode-provider` | Provider router，以及 OpenAI/Anthropic-compatible streaming adapters。 |
| `infinitecode-protocol` | 共享的 request、response、event、provider、permission、reference-search、command-exec、goal 和 session types。 |
| `infinitecode-config` | App、provider、server、tool、hook、MCP、skill 和 logging 设置的可序列化配置 contract。 |
| `infinitecode-tools` | Core、server 和 clients 共享的 tool contracts。 |
| `infinitecode-mcp` | 基于 RMCP 的 MCP runtime adapter。 |
| `infinitecode-code-search` | 本地语义代码检索实现。 |
| `infinitecode-file-search` | Composer reference search 使用的 fuzzy file search。 |
| `infinitecode-safety` | Approval 和 sandbox policy contracts。 |
...
3. `run_agent` 解析当前工作目录、`INFINITECODE_HOME`、effective config、model catalog、permission preset、provider settings 和 saved model entries。
4. `run_agent` 调用 `infinitecode_tui::run_interactive_tui`。
...
`infinitecode onboard` 使用同一条 setup 路径，但会强制进入 provider onboarding
mode，并在 provider/model binding 保存后退出。CLI 随后提示用户运行 `infinitecode`，
让下一个进程从新写入的配置启动。

`infinitecode resume <session_id>` 使用同一路径，并传入 initial session id。
...
Tools 在 server bootstrap path 中从配置注册，并由 core query runtime 执行。共享 tool contract 位于 `infinitecode-tools`；具体 runtime 行为位于 `infinitecode-core/src/tools` 下。
...
1. `infinitecode-config` 定义序列化的 provider 和 model binding 字段。
2. `infinitecode-core` 为当前工作区解析 effective provider settings。
3. `infinitecode-server` 在 bootstrap 时构建 provider router。
4. `infinitecode-provider` 通过选中的 wire API 发送请求。
...
UI state 放在 `infinitecode-tui`，runtime/session state 放在 `infinitecode-server`，agent 和 tool semantics 放在 `infinitecode-core`，provider-specific request/stream handling 放在 `infinitecode-provider`，serialized config shapes 放在 `infinitecode-config`。

当功能跨层时，先修改 `infinitecode-protocol` 中的共享 DTO，再按顺序更新 server handling、worker mapping 和 TUI rendering。
