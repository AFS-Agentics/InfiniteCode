---
title: 运行时流程
description: 跟踪一条用户消息如何经过 InfiniteCode。
---

这页跟踪一个普通用户 prompt 如何经过 InfiniteCode。
...
sequenceDiagram
  participant User
  participant TUI as infinitecode-tui
  participant Worker as TUI worker
  participant Server as infinitecode-server
  participant Core as infinitecode-core
  participant Tools as tools
  participant Provider as infinitecode-provider
...
Server 将 agent execution 委托给 `infinitecode-core`。
...
Interactive sessions 会持久化足够的 metadata 和 records，使 `infinitecode resume` 和 `/resume` 能重建历史。Resume 使用同一条 interactive startup path，但会把 initial session id 传入 TUI 和 worker。
