...
const statusTone = {
	yes: { text: "text-emerald-200", dot: "bg-emerald-300 shadow-[0_0_1.25rem_rgb(110_231_183_/_36%)]" },
	partial: { text: "text-amber-200", dot: "bg-amber-300 shadow-[0_0_1.25rem_rgb(252_211_77_/_30%)]" },
	no: { text: "text-white/46", dot: "bg-white/24" },
} as const;

function ProductMark({ product }: { product: string }) {
	const src = productLogos[product];
	if (src) {
		return <img alt="" className="h-6 w-6 object-contain" height={24} src={src} width={24} />;
	}
	return (
					...
					<table className="min-w-[72rem] border-collapse text-left">
						<thead>
							<tr className="border-b border-white/12">
								<th className="sticky left-0 z-20 w-56 bg-[#0a0f14] px-5 py-5 text-xs font-bold uppercase tracking-[0.16em] text-white/42" scope="col">
									{copy.capabilityLabel}
								</th>
								{copy.products.map((product, index) => (
									<th className={["w-60 px-5 py-5 text-sm font-semibold text-white", index === 0 ? "bg-[#111822]" : ""].join(" ")} key={product} scope="col">
						...
						<tbody>
							{copy.rows.map((row) => (
								<tr className="border-t border-white/8 align-top transition-colors hover:bg-white/[0.025]" key={row.capability}>
									<th className="sticky left-0 z-10 w-56 bg-[#0a0f14] px-5 py-5 text-sm font-semibold leading-6 text-white" scope="row">
										{row.capability}
									</th>
									{row.products.map((product, index) => {
										const tone = statusTone[product.status];
										return (
											<td className={["w-60 px-5 py-5", index === 0 ? "bg-[linear-gradient(180deg,rgb(255_148_31_/_9%),rgb(255_148_31_/_3%))]" : ""].join(" ")} key={`${row.capability}-${index}`}>
												...
												<p className="mt-2 text-xs leading-5 text-white/48">{product.evidence}</p>
											</td>
										);
									})}
								</tr>
