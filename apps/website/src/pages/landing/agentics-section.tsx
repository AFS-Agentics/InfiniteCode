...
function localizeHref(
	...
	if (!href) return href;
	// Locale prefix is intentionally not prepended — App.tsx currently only
	// registers `/docs/*`; a future change will route `/zh/docs/*` separately.
	void locale;
	return href;
}
	...
	if (kind === "search") {
		return (
			<svg aria-hidden="true" className={common} fill="none" viewBox="0 0 24 24">
				...
				<path d="M11 8v6M8 11h6" stroke="currentColor" strokeLinecap="round" strokeWidth="1.5" />
			</svg>
		);
	}
	...
	return (
		...
		>
			...
			<header className="relative flex items-center justify-between gap-4">
				<span className={`inline-flex h-10 w-10 items-center justify-center border ${accent.chip}`}>
			...
			<ul className="relative flex flex-col gap-2.5 border-t border-white/10 pt-5">
				{feature.bullets.map((bullet) => (
					<li
						...
						key={bullet}
					>
						<span aria-hidden="true" className={`mt-[0.55rem] h-1.5 w-1.5 ${accent.bullet}`} />
						<span>{bullet}</span>
					</li>
				))}
			</ul>
			...
			<div className="relative mx-auto max-w-7xl">
				...
				<div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
					{copy.features.map((feature) => (
						<FeatureCard feature={feature} key={feature.title} locale={locale} />
					))}
				</div>
			</div>
		</section>
