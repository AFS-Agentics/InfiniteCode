...
describe("ACP desktop SDK session mapping", () => {
			...
			if (method === "session/load") {
				tx?.emitSessionUpdate({
					...
					update: {
						...
						_meta: { "infinitecode/historyIndex": 0 },
					},
				} satisfies AcpSessionNotification)
				tx?.emitSessionUpdate({
					sessionId: "s1",
					update: {
						...
						kind: "read",
						status: "pending",
						_meta: { "infinitecode/historyIndex": 1, "infinitecode/parentMessageId": "history-0" },
					},
				} satisfies AcpSessionNotification)
				tx?.emitSessionUpdate({
					sessionId: "s1",
					update: {
						...
						status: "completed",
						rawOutput: "A done",
						_meta: { "infinitecode/historyIndex": 2, "infinitecode/parentMessageId": "history-0" },
					},
				} satisfies AcpSessionNotification)
				tx?.emitSessionUpdate({
					sessionId: "s1",
					update: {
						...
						_meta: { "infinitecode/historyIndex": 3, "infinitecode/parentMessageId": "history-0" },
					},
				} satisfies AcpSessionNotification)
				tx?.emitSessionUpdate({
					sessionId: "s1",
					update: {
						...
						_meta: { "infinitecode/historyIndex": 4 },
					},
				} satisfies AcpSessionNotification)
				tx?.emitSessionUpdate({
					sessionId: "s1",
					update: {
						...
						status: "completed",
						rawOutput: "B done",
						_meta: { "infinitecode/historyIndex": 5, "infinitecode/parentMessageId": "history-4" },
					},
				} satisfies AcpSessionNotification)
				tx?.emitSessionUpdate({
					sessionId: "s1",
					update: {
						...
						_meta: { "infinitecode/historyIndex": 6, "infinitecode/parentMessageId": "history-4" },
					},
				} satisfies AcpSessionNotification)
				return { configOptions }
			}
	...
	test("reparents same-turn assistant updates when the user echo arrives late", async () => {
		...
		transport.emitSessionUpdate({
			...
			update: {
				...
				_meta: { "infinitecode/turnId": "turn-live", "infinitecode/itemId": "assistant-live" },
			},
		} satisfies AcpSessionNotification)
		transport.emitSessionUpdate({
			sessionId: "s1",
			update: {
				...
				kind: "read",
				status: "pending",
				_meta: { "infinitecode/turnId": "turn-live", "infinitecode/itemId": "tool-live-item" },
			},
		} satisfies AcpSessionNotification)
		transport.emitSessionUpdate({
			sessionId: "s1",
			update: {
				...
				_meta: { "infinitecode/turnId": "turn-live", "infinitecode/itemId": "user-live" },
			},
		} satisfies AcpSessionNotification)
	...
	test("reads workspace changes through the runtime workspace API", async () => {
		const transport = new FakeTransport((method, params, directory) => {
			if (method === "_infinitecode/workspace/changes/read") {
	...
	test("maps original request_user_input events to questions and replies through runtime API", async () => {
		const transport = new FakeTransport((method) => {
			if (method === "initialize") return initializeResult
			if (method === "session/list") return { sessions: [sessionInfo] }
			if (method === "_infinitecode/request_user_input/respond") return { request_id: "rq1" }
			throw new Error(`unexpected request ${method}`)
		})
		const client = createInfiniteCodeClient({ directory: "/repo", transport })
		const stream = (await client.global.event()).stream[Symbol.asyncIterator]()
		...
		expect(transport.requests.at(-1)).toEqual({
			method: "_infinitecode/request_user_input/respond",
			directory: "/repo",
	...
	test("does not treat session info updatedAt as last activity without InfiniteCode metadata", async () => {
		const transport = new FakeTransport((method) => {
			if (method === "initialize") return initializeResult
			if (method === "session/list") return { sessions: [sessionInfo] }
			throw new Error(`unexpected request ${method}`)
	...
	test("updates session last activity from ACP activity metadata", async () => {
		...
		transport.emitSessionUpdate({
			...
			update: {
				...
				_meta: { "infinitecode/activityAt": "2026-06-24T00:05:00.000Z" },
			},
		} satisfies AcpSessionNotification)
		...
		transport.emitSessionUpdate({
			...
			update: {
				...
				status: "completed",
				rawOutput: "done",
				_meta: { "infinitecode/activityAt": "2026-06-24T00:06:00.000Z" },
			},
		} satisfies AcpSessionNotification)
	...
	test("uses ACP extension method for title updates and emits deletion events", async () => {
		const transport = new FakeTransport((method, params) => {
			if (method === "initialize") return initializeResult
			if (method === "session/list") return { sessions: [sessionInfo] }
			if (method === "_infinitecode/session/title/update") {
				return {
					session: {
	...
	test("emits local deletion events when a server broadcast reports deleted session ids", async () => {
		...
		transport.emitSessionUpdate({
			...
			update: { sessionUpdate: "session_info_update" },
			_meta: {
				"infinitecode/originalMethod": "session/deleted",
				"infinitecode/originalEvent": {
			...
			if (method === "session/list") {
				return {
					sessions: [
						{
							...sessionInfo,
							_meta: { "infinitecode/session": { status: "active_turn" } },
						},
					],
				}
			}
	...
	test("emits session status updates from wrapped original SessionStatusChanged events", async () => {
		...
		transport.emitSessionUpdate({
			...
			update: { sessionUpdate: "session_info_update" },
			_meta: {
				"infinitecode/originalMethod": "session/status/changed",
				"infinitecode/originalEvent": {
	...
	test("emits compaction lifecycle events from wrapped original events with nested session metadata", async () => {
		...
		transport.emitSessionUpdate({
			...
			update: { sessionUpdate: "session_info_update" },
			_meta: {
				"infinitecode/originalMethod": "session/compaction/started",
				"infinitecode/originalEvent": {
		...
		transport.emitSessionUpdate({
			...
			update: { sessionUpdate: "session_info_update" },
			_meta: {
				"infinitecode/originalMethod": "session/compaction/completed",
				"infinitecode/originalEvent": {
		...
		transport.emitSessionUpdate({
			...
			update: { sessionUpdate: "session_info_update" },
			_meta: {
				"infinitecode/originalMethod": "session/compaction/failed",
				"infinitecode/originalEvent": {
