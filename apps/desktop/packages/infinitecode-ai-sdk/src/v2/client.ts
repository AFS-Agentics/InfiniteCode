...
export interface InfiniteCodeAcpTransportEvent {
...
export interface InfiniteCodeAcpTransport {
...
export interface CreateInfiniteCodeClientOptions {
	baseUrl?: string
	directory?: string
	fetch?: typeof fetch
	transport?: InfiniteCodeAcpTransport
}
...
function sessionMeta(value: unknown): Record<string, unknown> | undefined {
	...
	return objectRecord(meta?.["infinitecode/session"])
}
...
function sessionStatusFromMetadata(value: unknown): string | undefined {
	const meta = objectRecord(value)
	const nestedStatus = objectRecord(meta?.["infinitecode/session"])?.status
	if (typeof nestedStatus === "string") return nestedStatus
	const directStatus = meta?.["infinitecode/session.status"]
...
const INFINITECODE_TURN_ID_META = "infinitecode/turnId"
const INFINITECODE_ACTIVITY_AT_META = "infinitecode/activityAt"
const INFINITECODE_HISTORY_INDEX_META = "infinitecode/historyIndex"
const INFINITECODE_PARENT_MESSAGE_ID_META = "infinitecode/parentMessageId"
const INFINITECODE_TURN_DURATION_MS_META = "infinitecode/turnDurationMs"
const INFINITECODE_ITEM_KIND_META = "infinitecode/itemKind"
const INFINITECODE_RESEARCH_ARTIFACT_TYPE_META = "infinitecode/researchArtifactType"
...
function textPartMetadataFromUpdate(
	...
	if (meta?.[INFINITECODE_ITEM_KIND_META] === "research_artifact") {
		metadata[INFINITECODE_ITEM_KIND_META] = "research_artifact"
		const artifactType = meta[INFINITECODE_RESEARCH_ARTIFACT_TYPE_META]
		if (typeof artifactType === "string" && artifactType) {
			metadata[INFINITECODE_RESEARCH_ARTIFACT_TYPE_META] = artifactType
		}
		const title = meta[INFINITECODE_RESEARCH_ARTIFACT_TITLE_META]
		if (typeof title === "string" && title) {
			metadata[INFINITECODE_RESEARCH_ARTIFACT_TITLE_META] = title
		}
	}
...
function updateHistoryCreatedAt(update: Record<string, unknown>): number | undefined {
	const value = updateMeta(update)?.[INFINITECODE_HISTORY_INDEX_META]
	const index =
		typeof value === "number"
			? value
			: typeof value === "string"
...
const initializePromises = new WeakMap<InfiniteCodeAcpTransport, Promise<void>>()
...
class AcpClient {
	private transport: InfiniteCodeAcpTransport | null = null
	...
	constructor(private readonly options: CreateInfiniteCodeClientOptions) {}
	project = {
	...
	session = {
		...
		update: async (params: { sessionID: string; title: string }) => {
			const result = (await this.request("_infinitecode/session/title/update", {
			...
			const session = this.rememberSession({
				...
				_meta: { "infinitecode/session": metadata },
			})
		...
		changes: {
			read: async (params: WorkspaceChangesReadOptions) => {
				...
				const data = (await this.request(
					"_infinitecode/workspace/changes/read",
					wireParams,
	...
	private async sessionMessages(sessionId: string, limit?: number): Promise<Array<{ info: Message; parts: Part[] }>> {
		...
		const result = (await this.request("session/load", {
			...
			additionalDirectories: [],
			mcpServers: [],
			...(limit === undefined ? {} : { _meta: { "infinitecode/historyLimit": limit } }),
	...
	private rememberSession(info: AcpSessionInfo): Session {
		...
		this.sessionStatuses.set(
			...
			metadataStatus === undefined
				? this.sessionStatuses.get(session.id) ?? statusFromInfiniteCode()
				: statusFromInfiniteCode(metadataStatus),
		)
		return session
	}
	...
	private async ensureInitialized(): Promise<void> {
		...
		await this.open()
		if (!this.transport) throw new Error("InfiniteCode ACP transport is not connected")
		if (this.initialized) return
	...
	/** InfiniteCode extension RPCs that are not yet present in the generated ACP schema bundle. */
	private async requestExtension(method: string, params: unknown): Promise<unknown> {
		await this.open()
		if (!this.transport) throw new Error("InfiniteCode ACP transport is not connected")
		const extensionMethod = method.startsWith("_infinitecode/") ? method : `_infinitecode/${method}`
	...
	private handleTransportEvent(event: InfiniteCodeAcpTransportEvent): void {
		...
		if (
			...
			(event.method === "workspace/changes/updated" ||
				event.method === "_infinitecode/workspace/changes/updated") &&
			event.params
		) {
	...
	private async respondToQuestion(
		...
		await this.request("_infinitecode/request_user_input/respond", respondParams)
	...
	private handleSessionUpdate(notification: AcpSessionNotification): void {
		...
		const deletedSessionIds = deletedSessionIdsFromOriginalEvent(
			notification._meta?.["infinitecode/originalEvent"],
		)
		...
		const activityAt = parseTimestampMs(updateMeta(update)?.[INFINITECODE_ACTIVITY_AT_META])
	...
	private handleOriginalEvent(
		...
		const original = notification._meta?.["infinitecode/originalEvent"]
		if (!original || typeof original !== "object") return
		const originalMethod =
			typeof notification._meta?.["infinitecode/originalMethod"] === "string"
				? notification._meta["infinitecode/originalMethod"]
				: undefined
	...
	private rememberSessionStatus(sessionId: string, directory: string, protocolStatus: string): void {
		const status = statusFromInfiniteCode(protocolStatus)
	...
	private turnIdForUpdate(update: Record<string, unknown>): string | undefined {
		return updateMetaString(update, INFINITECODE_TURN_ID_META)
	}
	...
	private parentMessageIdForUpdate(
		...
		return (
			updateMetaString(update, INFINITECODE_PARENT_MESSAGE_ID_META) ??
	...
	private applyHistoryTurnDuration(
		...
		const durationMs = Math.floor(
			numberFromProtocol(updateMeta(update)?.[INFINITECODE_TURN_DURATION_MS_META]),
		)
...
export type InfiniteCodeClient = any

export function createInfiniteCodeClient(options: CreateInfiniteCodeClientOptions = {}): InfiniteCodeClient {
