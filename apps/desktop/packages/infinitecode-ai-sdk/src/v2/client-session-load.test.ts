...
describe("ACP desktop SDK session cwd discovery", () => {
	...
	test("passes history limit through session/load meta and reloads larger windows", async () => {
		const transport = new FakeTransport((method, params, _directory, tx) => {
			...
			if (method === "session/list") return { sessions: [storedSession] }
			if (method === "session/load") {
				const limit = (params as { _meta?: Record<string, number> })._meta?.["infinitecode/historyLimit"]
				if (limit === 1) {
		...
		expect(transport.requests.filter((request) => request.method === "session/load")).toEqual([
			{
				...
				params: {
					...
					additionalDirectories: [],
					mcpServers: [],
					_meta: { "infinitecode/historyLimit": 1 },
				},
			},
			{
				method: "session/load",
				...
				params: {
					...
					additionalDirectories: [],
					mcpServers: [],
					_meta: { "infinitecode/historyLimit": 2 },
				},
			},
		])
	})
			...
			if (method === "session/load") {
				...
				tx?.emitSessionUpdate({
					...
					update: {
						...
						_meta: {
							"infinitecode/historyIndex": 2,
							"infinitecode/parentMessageId": "history-0",
							"infinitecode/turnDurationMs": 42_000,
						},
					},
				} satisfies AcpSessionNotification)
				return {}
			...
			if (method === "session/load") {
				...
				tx?.emitSessionUpdate({
					...
					update: {
						...
						_meta: {
							"infinitecode/historyIndex": 1,
							"infinitecode/parentMessageId": "history-0",
							"infinitecode/itemKind": "research_artifact",
							"infinitecode/researchArtifactType": "brief",
							"infinitecode/researchArtifactTitle": "Research Brief",
						},
					},
				} satisfies AcpSessionNotification)
				return {}
			...
			if (method === "session/load") {
				...
				tx?.emitSessionUpdate({
					...
					update: {
						...
						_meta: { "infinitecode/historyIndex": 2 },
					},
				} satisfies AcpSessionNotification)
				tx?.emitSessionUpdate({
					sessionId: "stored-session",
					update: {
						...
						_meta: { "infinitecode/historyIndex": 3, "infinitecode/parentMessageId": "history-2" },
					},
				} satisfies AcpSessionNotification)
				tx?.emitSessionUpdate({
					sessionId: "stored-session",
					update: {
						...
						_meta: { "infinitecode/historyIndex": 4, "infinitecode/parentMessageId": "history-2" },
					},
				} satisfies AcpSessionNotification)
				return {}
			}
