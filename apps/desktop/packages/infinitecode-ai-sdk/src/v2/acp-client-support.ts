...
import type { InfiniteCodeAcpTransport } from "./client"
...
let sharedIpcTransport: InfiniteCodeAcpTransport | null = null

export function createIpcTransport(): InfiniteCodeAcpTransport {
	if (sharedIpcTransport) return sharedIpcTransport

	const api = globalThis.window?.infinitecode?.acp
	if (!api) throw new Error("window.infinitecode.acp is not available")
	sharedIpcTransport = {
...
export function statusFromInfiniteCode(status?: string): any {
