...
import {
	...
	SheetHeader,
	SheetTitle,
} from "@infinitecode/ui/components/sheet"
...
function SidebarProvider({
	...
	/** When true, the sidebar fits within a parent flex container instead of using fixed/svh positioning. */
	embedded?: boolean
}) {
	...
	const contextValue = React.useMemo<SidebarContextProps>(
		...
		[state, open, setOpen, isMobile, openMobile, setOpenMobile, toggleSidebar, embedded],
	)
...
function Sidebar({
	...
	const { isMobile, state, openMobile, setOpenMobile, embedded } = useSidebar()
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
	return (
		<main
			...
			className={cn(
				"bg-background md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow-sm md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2 relative flex min-w-0 flex-1 flex-col overflow-hidden",
				className,
			)}
			{...props}
		/>
