/**
 * Canonical -> InfiniteCode format converter.
 *
 * Produces InfiniteCode-compatible configuration files from canonical representation.
 */
import type {
	CanonicalAgentFile,
...
/**
 * Convert canonical scan result to InfiniteCode output files.
 */
export function canonicalToInfiniteCode(
...
function convertProjectToOC(
	...
	if (scopedRules.length > 0) {
		report.manualActions.push(
			`${scopedRules.length} file-scoped/intelligent rules found in ${project.path}. ` +
				`InfiniteCode does not support file-scoped rules natively. ` +
	...
	for (const agent of project.agents) {
		...
		result.agents.set(`${project.path}/.infinitecode/agents/${agent.name}.md`, content)
	...
	for (const cmd of project.commands) {
		...
		result.commands.set(`${project.path}/.infinitecode/commands/${cmd.name}.md`, content)
...
function convertCommandToOC(cmd: CanonicalCommandFile): {
	...
	const fm: InfiniteCodeCommandFrontmatter = {
