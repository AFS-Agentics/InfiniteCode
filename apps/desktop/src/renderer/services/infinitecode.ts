...
import { createInfiniteCodeClient } from "@infinitecode-ai/sdk/v2/client"
import { stableId } from "@infinitecode-ai/sdk/v2/acp-client-support"
import type { Event, InfiniteCodeProject, QuestionAnswer, Session, SessionStatus } from "../lib/types"
import { createLogger } from "../lib/logger"
import { workspacePatchFilesFromView } from "../lib/workspace-diff"

export type { InfiniteCodeClient }

const log = createLogger("infinitecode-service")
...
/**
 * Creates a InfiniteCode client over the preload ACP bridge.
 */
export function connectToServer(url: string, options?: ConnectOptions): InfiniteCodeClient {
	void url
	void options?.authHeader
	return createInfiniteCodeClient({ directory: options?.directory })
}

/**
 * Fetch all projects known to the server.
 */
export async function listProjects(client: InfiniteCodeClient): Promise<InfiniteCodeProject[]> {
...
export function projectsFromSessions(
	...
	const byDirectory = new Map<string, InfiniteCodeProject>()
...
function clientDirectory(client: InfiniteCodeClient): string | undefined {
 ...
 */
export async function createSession(client: InfiniteCodeClient, title?: string): Promise<Session> {
 ...
 */
export async function abortSession(client: InfiniteCodeClient, sessionId: string): Promise<void> {
 ...
 * Delete a session.
 */
export async function deleteSession(client: InfiniteCodeClient, sessionId: string): Promise<void> {
 ...
 */
export async function getSession(client: InfiniteCodeClient, sessionId: string): Promise<Session | null> {
	try {
 ...
 */
export async function getSessionDiff(client: InfiniteCodeClient, sessionId: string) {
 ...
 */
export async function rejectQuestion(client: InfiniteCodeClient, requestId: string): Promise<void> {
	await client.question.reject({ requestID: requestId })
}

/**
 * Dispose a specific project instance on the InfiniteCode server.
 ...
 */
export async function disposeInstance(client: InfiniteCodeClient): Promise<void> {
	await client.instance.dispose()
}

/**
 * Dispose all instances on the InfiniteCode server (global reload).
 ...
 */
export async function disposeAllInstances(client: InfiniteCodeClient): Promise<void> {
 ...
 */
export async function unrevertSession(client: InfiniteCodeClient, sessionId: string): Promise<Session> {
 ...
 */
export async function findFiles(client: InfiniteCodeClient, query: string): Promise<string[]> {
 ...
 */
export async function summarizeSession(client: InfiniteCodeClient, sessionId: string): Promise<void> {
 ...
 */
export async function getSessionMessages(client: InfiniteCodeClient, sessionId: string) {
