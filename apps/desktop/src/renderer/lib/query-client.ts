 ...
 * and imperative code (e.g. the ACP event processor that needs to invalidate queries).
 */
export const queryClient = new QueryClient({
	defaultOptions: {
		queries: {
			...
			gcTime: 10 * 60 * 1000,
			// Don't refetch on window focus by default — ACP events handle real-time updates.
