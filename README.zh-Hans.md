...
**InfiniteCode 是开源 coding agent，提供 Desktop app、终端 TUI/CLI 和模型中立的 Rust runtime，面向私有化、企业内网和 OpenAI 兼容模型环境。可接入 DeepSeek、Qwen、Kimi、Anthropic 兼容 API、本地网关或自定义模型端点。**
...
[为什么选择 InfiniteCode](#为什么选择-infinitecode) · [截图](#截图) · [功能](#功能) · [已测试模型](#已测试模型) · [已测试平台](#已测试平台) · [安装](#安装) · [快速开始](#快速开始) · [文档](#docs)
...
<p align="center">
  <img width="100%" alt="InfiniteCode desktop coding agent app 展示仓库对话、项目侧边栏和模型控制" src="./.github/assets/infinitecode-desktop-coding-agent-screenshot.png" />
</p>

<p align="center">
  <img width="100%" alt="InfiniteCode 终端 TUI coding agent 在本地仓库中显示模型、上下文和 token 状态" src="./.github/assets/infinitecode-terminal-tui-coding-agent-screenshot.png" />
</p>

## 为什么选择 InfiniteCode

InfiniteCode 面向那些不想绑定单一托管模型生态、需要掌控模型选择、运行时行为和
...
- **长任务支持** - 让 InfiniteCode 在多轮工作中自动管理上下文，避免任务变长后丢失上下文。
...
InfiniteCode 的内置模型目录包含 Qwen、Kimi、MiniMax、GLM 和 DeepSeek 的已测试模型定义。
...
InfiniteCode 已在 macOS、Linux、Windows 和麒麟操作系统上测试。
...
InfiniteCode 有两种安装形态。需要图形化 coding agent 工作区时选择 Desktop app；
偏好 shell-first 开发时选择终端原生的 TUI/CLI；也可以在同一台机器上同时安装两者。

### 选项一：Desktop App

如果你想使用图形化 InfiniteCode 体验，请从
[GitHub Releases](https://github.com/AFS-Agentics/InfiniteCode/releases/latest)
下载最新的 InfiniteCode Desktop 安装包，并选择与你的操作系统和架构匹配的 asset：

- **macOS** - 下载 `infinitecode-desktop-...-mac-...` 的 `.dmg` 或 `.zip` asset。
- **Windows** - 下载 `infinitecode-desktop-...-windows-...` 的 `.exe` asset。
- **Linux** - 下载 `infinitecode-desktop-...-linux-...` 的 `.AppImage`、`.deb`
  或 `.rpm` asset。

**如果 macOS 显示以下错误，这是正常现象：“InfiniteCode”已损坏，无法打开。**当前
...
在线安装器会把 `infinitecode` 放到 InfiniteCode home 目录下，安装用于快速仓库搜索的
...
```bash
infinitecode                         # 在当前仓库启动交互式 TUI
infinitecode resume <session-id>
```

## 配置

`infinitecode onboard` 是推荐的设置路径。如需手动 `config.toml` 路径、
...
InfiniteCode 仍处于 1.0 之前并在积极开发中。它已经适合本地评估、实验和贡献者使用；
公共 API 和配置仍可能变化。
...
请使用 TUI/CLI。两种入口都面向同一个本地 InfiniteCode runtime。
...
**如果你觉得 InfiniteCode 有用，请考虑给它一个 star。**
