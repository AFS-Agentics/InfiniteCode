...
**InfiniteCode 是開源 coding agent，提供 Desktop app、終端機 TUI/CLI 和模型中立的 Rust runtime，面向私有化、企業內網和 OpenAI 相容模型環境。可接入 DeepSeek、Qwen、Kimi、Anthropic 相容 API、本地閘道或自訂模型端點。**
...
[為什麼選擇 InfiniteCode](#為什麼選擇-infinitecode) · [截圖](#截圖) · [功能](#功能) · [已測試模型](#已測試模型) · [已測試平台](#已測試平台) · [安裝](#安裝) · [快速開始](#快速開始) · [文件](#docs)
...
<p align="center">
  <img width="100%" alt="InfiniteCode desktop coding agent app 展示儲存庫對話、專案側邊欄和模型控制" src="./.github/assets/infinitecode-desktop-coding-agent-screenshot.png" />
</p>

<p align="center">
  <img width="100%" alt="InfiniteCode 終端機 TUI coding agent 在本地儲存庫中顯示模型、上下文和 token 狀態" src="./.github/assets/infinitecode-terminal-tui-coding-agent-screenshot.png" />
</p>

## 為什麼選擇 InfiniteCode

InfiniteCode 面向那些不想綁定單一託管模型生態、需要掌控模型選擇、執行階段行為和
...
- **長時間任務支援** - 讓 InfiniteCode 在多輪工作中自動管理上下文，避免任務變長後丟失脈絡。
...
InfiniteCode 的內建模型目錄包含 Qwen、Kimi、MiniMax、GLM 和 DeepSeek 的已測試模型定義。
...
InfiniteCode 已在 macOS、Linux、Windows 和麒麟作業系統上測試。
...
InfiniteCode 有兩種安裝形態。需要圖形化 coding agent 工作區時選擇 Desktop app；
偏好 shell-first 開發時選擇終端機原生的 TUI/CLI；也可以在同一台機器上同時安裝兩者。

### 選項一：Desktop App

如果你想使用圖形化 InfiniteCode 體驗，請從
[GitHub Releases](https://github.com/AFS-Agentics/InfiniteCode/releases/latest)
下載最新的 InfiniteCode Desktop 安裝包，並選擇與你的作業系統和架構匹配的 asset：

- **macOS** - 下載 `infinitecode-desktop-...-mac-...` 的 `.dmg` 或 `.zip` asset。
- **Windows** - 下載 `infinitecode-desktop-...-windows-...` 的 `.exe` asset。
- **Linux** - 下載 `infinitecode-desktop-...-linux-...` 的 `.AppImage`、`.deb`
  或 `.rpm` asset。

**如果 macOS 提示「InfiniteCode」已損壞，無法打開，這是正常現象。**目前 macOS
...
線上安裝器會把 `infinitecode` 放到 InfiniteCode home 目錄下，安裝用於快速儲存庫搜尋的
...
```bash
infinitecode                         # 在目前儲存庫啟動互動式 TUI
infinitecode resume <session-id>
```

## 配置

`infinitecode onboard` 是推薦的設定路徑。如需手動 `config.toml` 路徑、
...
InfiniteCode 仍處於 1.0 之前並在積極開發中。它已經適合本地評估、實驗和貢獻者使用；
公共 API 和配置仍可能變更。
...
請使用 TUI/CLI。兩種入口都面向同一個本地 InfiniteCode runtime。
...
**如果你覺得 InfiniteCode 有用，請考慮給它一個 star。**
